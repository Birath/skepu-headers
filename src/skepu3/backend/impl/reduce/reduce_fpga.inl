/*! \file reduce_fpga.inl
*  \brief Contains the definitions of FPGA specific member functions for the Reduce skeleton.
 */
...
namespace skepu
{
	namespace backend
	{
		...
		::reduceSingleThreadOneDim_FPGA(size_t deviceID, VectorIterator<T> &res, const MatrixIterator<T> &arg, size_t numRows)
		{
			...
			for (size_t r = 0; r < rows; r++)
			{
				if (r > 0)
				{
					...
					cl_int err;
					d_input  = clCreateSubBuffer(deviceInPointer,  CL_MEM_READ_WRITE, CL_BUFFER_CREATE_TYPE_REGION, &inRegion,  err);
					CL_CHECK_ERROR(err, "Failed to create input SubBuffer");
					d_output = clCreateSubBuffer(deviceMemPointer, CL_MEM_READ_WRITE, CL_BUFFER_CREATE_TYPE_REGION, &outRegion, err);
					CL_CHECK_ERROR(err, "Failed to create output SubBuffer");
				}
		...
		::FPGA(VectorIterator<T> &res, const MatrixIterator<T>& arg, size_t numRows)
		{
			...
			if (numDevices <= 1) {
				this->reduceSingleThreadOneDim_FPGA(0, res, arg, numRows);
				return;
			}
			
#endif // SKEPU_DEBUG_FORCE_MULTI_GPU_IMPL
			
			this->reduceMultipleOneDim_FPGA(numDevices, res, arg, numRows);
		}
		...
		::FPGA(size_t size, T &res, Iterator arg)
		{
			...
			if (numDevices <= 1)
				return this->reduceSingle_FPGA(0, size, res, arg);
			
#endif // SKEPU_DEBUG_FORCE_MULTI_GPU_IMPL
			
			return this->reduceMultiple_FPGA(numDevices, size, res, arg);
		}
		...
		::reduceSingle_FPGA(size_t deviceID, T &res, const MatrixIterator<T>& arg, size_t numRows)
		{
			...
			for (size_t r = 0; r < rows; r++)
			{
				...
				// execute the reduction for the given row
				ExecuteReduceOnFPGA<T>(deviceID, CLKernel::reduceRowWise, cols, numThreads, numBlocks, d_input, d_output);
			...
			if (rows > REDUCE_GPU_THRESHOLD)
			{
				...
				ExecuteReduceOnFPGA<T>(deviceID, CLKernel::reduceColWise, rows, numThreads, numBlocks, d_input, d_output);
		...
		::reduceNumDevices_FPGA(size_t numDevices, T &res, const MatrixIterator<T>& arg, size_t numRows)
		{
			...
			for (size_t i = 0; i < numDevices; ++i)
			{
				...
				for (size_t r = 0; r < numRows; r++)
				{
					...
					// execute the reduction for the given row
					ExecuteReduceOnFPGA<T>(i, CLKernel::reduceRowWise, cols, numThreads, numBlocks, d_input, d_output);
			...
			if (rows > REDUCE_GPU_THRESHOLD)
			{
				...
				// execute the reduction for the resulting row
				std::tie(numThreads, numBlocks) = getNumBlocksAndThreads(rows, this->m_selected_spec->GPUBlocks(), this->m_selected_spec->GPUThreads());
				ExecuteReduceOnFPGA<T>(0, CLKernel::reduceColWise, rows, numThreads, numBlocks, d_input, d_output);
		...
		::FPGA(T &res, const MatrixIterator<T>& arg, size_t numRows)
		{
			...
			if (numDevices <= 1)
				return this->reduceSingle_FPGA(0, res, arg, numRows);
			
#endif // SKEPU_DEBUG_FORCE_MULTI_GPU_IMPL
			
			return this->reduceNumDevices_FPGA(numDevices, res, arg, numRows);
		}
